<div class="footer-direction">
  <table>
    <tr>
      <td colspan='2' class="firstboxFd">
        <div class="icon-fd">
          <img src="/wp-content/uploads/2020/12/icon-dich-vu-dat-lich-hover-1.png" />
        </div>
        <div class="date-time-td">
          Mở cửa: 8h - 20h
        </div>
        <div class="small-text-td">
          Tất cả các ngày trong tuần kể cả ngày lễ
        </div>
      </td>
    </tr>
    <tr>
      <td class="item-fd">
        <div class="item-inner-fd left">
          <table>
            <tr>
              <td class="small-icon-fd">
                <img src="/wp-content/uploads/2021/01/homefd.png" />
              </td>
              <td class="name-fd">Trang chủ</td>
            </tr>
          </table>
        </div>
      </td>
      <td class="item-fd">
        <div class="item-inner-fd">
          <table>
            <tr>
              <td class="small-icon-fd">
                <img src="/wp-content/uploads/2021/01/introfd.png" />
              </td>
              <td class="name-fd">Giới thiệu</td>
            </tr>
          </table>
        </div>
      </td>
    </tr>
    <tr>
      <td class="item-fd">
        <div class="item-inner-fd left">
          <table>
            <tr>
              <td class="small-icon-fd">
                <img src="/wp-content/uploads/2021/01/docktorfd.png" />
              </td>
              <td class="name-fd">Đội ngũ bác sĩ</td>
            </tr>
          </table>
        </div>
      </td>
      <td class="item-fd">
        <div class="item-inner-fd">
          <table>
            <tr>
              <td class="small-icon-fd">
                <img src="/wp-content/uploads/2021/01/plusfd.png" />
              </td>
              <td class="name-fd">Cơ sở vật chất</td>
            </tr>
          </table>
        </div>
      </td>
    </tr>
    <tr>
      <td class="item-fd">
        <div class="item-inner-fd left">
          <table>
            <tr>
              <td class="small-icon-fd">
                <img src="/wp-content/uploads/2021/01/vus1fd.png" />
              </td>
              <td class="name-fd">Bệnh nam khoa</td>
            </tr>
          </table>
        </div>
      </td>
      <td class="item-fd">
        <div class="item-inner-fd">
          <table>
            <tr>
              <td class="small-icon-fd">
                <img src="/wp-content/uploads/2021/01/vus2fd.png" />
              </td>
              <td class="name-fd">Bệnh xã hội</td>
            </tr>
          </table>
        </div>
      </td>
    </tr>
    <tr>
      <td colspan='2' class='lastboxFd <?php if(is_page('363')) echo "lasthome-363"; ?>'>
        <div class="thongtinphongkham-fd">
          THÔNG TIN PHÒNG KHÁM
        </div>
        <div class="info-fd">
          <i class="fa fa-phone" aria-hidden="true"></i> <span class="detail-info">02X XXX XXXX</span>
        </div>
        <div class="info-fd">
          <i class="fa fa-map-marker" aria-hidden="true"></i> <span class="detail-info">QUẬN 10, TPHCM</span>
        </div>
        <div class="info-fd">
          <i class="fa fa-envelope-o" aria-hidden="true"></i> <span class="detail-info">dakhoaxx@gmail.com</span>
        </div>
      </td>
    </tr>
    <?php if (is_page('363')) { ?>
      <tr>
        <td colspan='2'>
        <div class="attention-note attent-363">
          (*) Lưu ý : Hiệu quả hổ trợ điều trị phụ thuộc vào cơ địa của mỗi người
        </div>
        </td>
      </tr>
    <?php } ?>
  </table>
</div>